...
fun NavGraphBuilder.myGamesGraphNavigation() {
    navigation(
        startDestination = GameGraphRoute.MyGamesPage.route,
        route = MrXGraph.GAMES,
    ) {
        composable(route = GameGraphRoute.MyGamesPage.route) {
            MyGamesScreen()
        }
        composable(
            route = GameGraphRoute.InGame.route.plus("/{$GAME_ID}"),
            arguments =
                listOf(
                    navArgument(
                        name = GAME_ID,
        ...
        composable(route = GameGraphRoute.ScanQrCode.route) {
            ScanQrCodeScreen()
        }
    }
}

sealed class GameGraphRoute(
    val route: String,
) {
    data object MyGamesPage : GameGraphRoute("MY_GAMES_PAGE")

    data object ScanQrCode : GameGraphRoute("SCAN_QR_CODE")

    data object InGame : GameGraphRoute("IN_GAME") {
